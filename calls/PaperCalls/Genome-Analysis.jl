...
# Group with the most CG sites has X CpGs
...
# 2. How many of those contain 1 CG-group: 12
sum(ar_data[:,3] .== 1.0)
...
# 4. The max and min size in # bases of the regions containing at least 2 CG-groups: (3898,4120)
ind = ar_data[:,3] .>= 2.0;
minimum(ar_data[ind,1])
maximum(ar_data[ind,1])
